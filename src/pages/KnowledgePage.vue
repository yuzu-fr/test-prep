...
function startReview() {
  ...
  shuffledCards.value = [...pool].sort(() => Math.random() - 0.5)
  reviewIndex.value = 0
...
function nextReview() {
  if (reviewIndex.value < shuffledCards.value.length - 1) {
    reviewIndex.value++
    isReviewFlipped.value = false
  }
}
  ...
  <main class="container">
    ...
    <header class="page-header">
      <div class="header-content">
        ...
        <p class="subtitle" v-if="viewMode === 'timeline'">Explorez les jalons de l'histoire de France</p>
        <p class="subtitle" v-else>Activez votre mémoire avec les flashcards</p>
      </div>
        ...
        <div class="year-list">
          <div v-for="(cards, year) in years" :key="year" class="year-row">
            ...
            <div v-if="cards.length === 1" class="event-card" @click="openCard(cards[0].card_id)">
              ...
              <span class="title-label">{{ cards[0].title_fr }}</span>
            </div>
              ...
              <transition name="slide">
                <div v-if="expandedYears.has(`${period}-${year}`)" class="sub-events">
                  <div v-for="card in cards.slice(1)" :key="card.card_id" class="event-card sub" @click="openCard(card.card_id)">
                    <span class="title-label">{{ card.title_fr }}</span>
                  </div>
                </div>
              </transition>
            </div>
    ...
    <div v-else class="review-view fade-in">
      <div v-if="shuffledCards.length > 0" class="review-container">
        <div class="review-meta">
          <span class="progress-text">Carte {{ reviewIndex + 1 }} sur {{ shuffledCards.length }}</span>
          <span class="difficulty-indicator" :class="'diff-' + shuffledCards[reviewIndex].difficulty">
            Niveau {{ shuffledCards[reviewIndex].difficulty }}
          </span>
        </div>
          ...
          <div class="flashcard" :class="{ flipped: isReviewFlipped }" @click="isReviewFlipped = !isReviewFlipped">
            <div class="card-inner">
              ...
              <div class="card-face front">
                ...
                <div class="card-body prompt-list">
                  <template v-if="Array.isArray(shuffledCards[reviewIndex].front_prompt)">
                    <p v-for="(p, idx) in shuffledCards[reviewIndex].front_prompt" :key="idx" class="prompt-item">
                      {{ p.fr }}
                    </p>
                  </template>
                  <p v-else>{{ shuffledCards[reviewIndex].front_prompt.fr }}</p>
                </div>
              ...
              <div class="card-face back">
                ...
                <div class="card-body">
                  <p>{{ shuffledCards[reviewIndex].core_fr }}</p>
                </div>
        ...
        <div class="review-actions">
          <button class="btn-primary main-action" @click="nextReview" :disabled="reviewIndex === shuffledCards.length - 1">
            {{ reviewIndex === shuffledCards.length - 1 ? 'Session Terminée' : 'Question Suivante →' }}
          </button>
        </div>
      </div>
            ...
            <div class="card-inner">
              <div class="card-face front">
                <div class="modal-header">
                  ...
                  <span class="modal-title">{{ activeCard.title_fr }}</span>
                </div>
                <div class="card-label">QUESTIONS POSSIBLES</div>
                <div class="card-body prompt-list">
                  <template v-if="Array.isArray(activeCard.front_prompt)">
                    <p v-for="(p, idx) in activeCard.front_prompt" :key="idx" class="prompt-item">
                      {{ p.fr }}
                    </p>
                  </template>
                  <p v-else>{{ activeCard.front_prompt.fr }}</p>
                </div>
